...
if __name__ == '__main__':
    ...
    # On Windows, the Werkzeug reloader commonly causes double-start issues.
    # Keep debug logs, but disable the reloader by default.
    debug = _truthy_env("FLASK_DEBUG", "1")
    use_reloader = _truthy_env("FLASK_USE_RELOADER", "0")
    skip_startup_db_check = _truthy_env("SKIP_STARTUP_DB_CHECK", "0")

    # Initial check for admin user + warp init.
    # This is best-effort and time-bounded so the server can still start if MongoDB/DNS is unavailable.
    if not skip_startup_db_check:
        try:
            from concurrent.futures import ThreadPoolExecutor, TimeoutError

            with ThreadPoolExecutor(max_workers=1) as executor:
                future = executor.submit(get_db_connection)
                try:
                    client, db, loom_collection, users_collection, warp_data_collection, warp_history_collection = future.result(timeout=8)
                except TimeoutError:
                    app.logger.critical(
                        "Startup DB check timed out. Continuing without startup initialization. "
                        "Set SKIP_STARTUP_DB_CHECK=1 to silence this in dev."
                    )
                    client = db = loom_collection = users_collection = warp_data_collection = warp_history_collection = None
            ...
            if client and users_collection is not None:
                if users_collection.count_documents({}) == 0:
                    ...
                    app.logger.info(
                        f"Default admin user '{default_admin_username}' created. PLEASE CHANGE THE DEFAULT PASSWORD IN PRODUCTION!"
                    )

                # Initialize current warp if it doesn't exist
                if warp_data_collection is not None and warp_data_collection.find_one({"_id": "current_warp"}) is None:
                ...
                app.logger.critical(
                    "Could not connect to database at startup to check/create default admin and initialize warp data. "
                    "The server will still run, but DB-backed endpoints will fail until MongoDB is reachable."
                )
        except Exception as e:
            app.logger.critical(
                f"Startup DB check failed unexpectedly: {e}. Continuing without startup initialization.",
                exc_info=True,
            )
